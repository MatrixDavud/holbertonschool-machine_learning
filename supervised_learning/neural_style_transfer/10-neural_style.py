...
class NST:
    ...
    def layer_style_cost(self, style_output, gram_target):
        ...
        if not (isinstance(gram_target, tf.Tensor) or isinstance(gram_target, tf.Variable)) or gram_target.shape.dims != [m, nc, nc]:
            raise TypeError(
                'gram_target must be a tensor of shape [{}, {}, {}]'.format(m, nc, nc))
    ...
    def style_cost(self, style_outputs):
        ...
        J_style = tf.add_n([self.layer_style_cost(
            style_outputs[i], self.gram_style_features[i]) for i in range(len(style_outputs))])
    ...
    def total_cost(self, generated_image):
        if not (isinstance(generated_image, tf.Tensor) or isinstance(generated_image, tf.Variable)) or generated_image.shape.dims != self.content_image.shape.dims:
            raise TypeError('generated_image must be a tensor of shape {}'.format(
    ...
    def generate_image(self, iterations=1000, step=None, lr=0.01, beta1=0.9, beta2=0.99):
        ...
        if step is not None:
            print("Cost at iteration {}: {}, content {}, style {}, var {}".format(
                iterations, J, J_content, J_style, J_var))
