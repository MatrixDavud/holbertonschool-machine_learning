...
class NST:
    ...
    def scale_image(image):
        ...
        image = tf.clip_by_value(
            tf.image.resize(image, new_shape, 'bicubic') / 255, 0, 1)
        return image
    ...
    def generate_features(self):
        ...
        pre_s = tf.keras.applications.vgg19.preprocess_input(
            self.style_image * 255)
        pre_c = tf.keras.applications.vgg19.preprocess_input(
            self.content_image * 255)
        style_features = self.model(pre_s)[:-1]
        self.content_feature = self.model(pre_c)[-1]
    ...
    def layer_style_cost(self, style_output, gram_target):
        ...
        if not (isinstance(style_output, (tf.Tensor, tf.Variable))
                and style_output.shape.ndims == 4):
        ...
        if not (isinstance(gram_target, (tf.Tensor, tf.Variable)) and
            ...
            raise TypeError(
                f'gram_target must be a tensor of shape [{m}, {nc}, {nc}]')
    ...
    def total_cost(self, generated_image):
        ...
        style_outputs = [s for s in model_outputs[:-1]]
    ...
    def compute_grads(self, generated_image):
        """Compute gradients of total cost wrt generated image."""
        if not (isinstance(generated_image, (tf.Tensor, tf.Variable)) and
                generated_image.shape.dims ==
                self.content_image.shape.dims):
            raise TypeError(
    ...
    def generate_image(self, iterations=1000, step=None,
        ...
        for i in range(iterations):
            ...
            if step is not None and i % step == 0:
                print(
                    f"Cost at iteration {i}: {J}, content {J_content}, "
        ...
        if step is not None:
            print(
                f"Cost at iteration {iterations}: {J}, content {J_content}, "
                f"style {J_style}, var {J_var}"
            )
